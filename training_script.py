...
def training_loop(nb_max_epochs, patience, verbose=1):
...
if __name__ == "__main__":
  ...
  for i in range(nb_of_trainings):
    ...
    model = GCN(num_node_features=dataset.num_node_features, 
                ...
                local_pool_method=local_pooling_selection(local_pooling_layer)).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = torch.nn.CrossEntropyLoss()

    # Model training
    best_model, test_acc = training_loop(max_epochs, patience)
    if verbose > 0:
